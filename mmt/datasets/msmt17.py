...
## zdy  ##
def _pluck_msmt_train(list_file, subdir, pattern=re.compile(r'([-\d]+)_([-\d]+)_([-\d]+)')):
    with open(list_file, 'r') as f:
        lines = f.readlines()
    ret = []
    pids = []
    for img_idx, line in enumerate(lines):
        ...
        if pid not in pids:
            pids.append(pid)
        ret.append((img_idx, osp.join(subdir,fname), pid, cam))
    return ret, pids
## zdy  ##
...
class Dataset_MSMT(object):
    ...
    def load(self, verbose=True):
        ...
        self.train, train_pids = _pluck_msmt_train(osp.join(exdir, 'list_train.txt'), 'train')
        self.val, val_pids = _pluck_msmt_train(osp.join(exdir, 'list_val.txt'), 'train')
