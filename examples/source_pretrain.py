...
def get_data(name, data_dir, height, width, batch_size, workers, num_instances, iters=200):
    ...
    train_loader = IterLoader(
                DataLoader(Preprocessor(train_set, root=dataset.images_dir,
                                        transform=train_transformer,mutual=False,cluster=False,pretrain=True),       ###   zdy
                            batch_size=batch_size, num_workers=workers, sampler=sampler,
                            shuffle=not rmgs_flag, pin_memory=True, drop_last=True), length=iters)

    test_loader = DataLoader(
        Preprocessor(list(set(dataset.query) | set(dataset.gallery)),
                     root=dataset.images_dir, transform=test_transformer,mutual=False,cluster=False,pretrain=False), ###   zdy
